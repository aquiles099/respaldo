 ...
 */
 ...
 var createLoad = function ()
 {
   ...
   $('#divButton').html("<button type='submit' onclick = 'this.disabled = true' id='loginButton' class='btn btn-primary pull-right'><i class='fa fa-circle-o-notch fa-spin'></i> Espere...</button>");
 }
...
var icsSearchPrealert = function () {
  ...
  var url = window.location.origin + window.location.pathname + '/verify';
...
var icsSetDataLoadSelect = function (json) {
  ...
  */
  ...
  $.each(json.courier, function(i, item) {
    if(item.id == courier){
      $('#courierSelect').append("<option value='"+item.id+"' selected> "+item.name+"</option");
    }
  });

  $.each(json.courier, function(i, item) {
    if(item.id != courier){
     $('#courierSelect').append("<option value='"+item.id+"'> "+item.name+"</option");
    }
  });
