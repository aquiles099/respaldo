        ...
        @foreach ($packages as $package)
            <tr>
              ...
              <td style="vertical-align:middle">
                ...
                @if($package->getLastEvent->step == HConstants::EVENT_ARRIVED) {{trans('package.available')}} <i class="fa fa-check" aria-hidden="true"></i>@endif
              ...
              <td style="background-color:white; vertical-align:middle">
                <ul class="nav nav-wizard" style="padding-left: 6%">
                  <li @if($package->getLastEvent->step == HConstants::EVENT_RECEIVED) class="active" @endif><a>{{trans('package.received')}}</a></li>
                  <li @if($package->getLastEvent->step == HConstants::EVENT_PROCESED) class="active" @endif><a>{{trans('package.processed')}}</a></li>
                  <li @if($package->getLastEvent->step == HConstants::EVENT_TRANSIT) class="active" @endif><a>{{trans('package.inTransit')}}</a></li>
                  <li @if($package->getLastEvent->step == HConstants::EVENT_ARRIVED) class="active" @endif><a>{{trans('package.available')}}</a></li>
                  <li @if($package->getLastEvent->step == HConstants::EVENT_AVAILABLE) class="active" @endif><a>{{trans('package.delivered')}}</a></li>
                </ul>
              </td>
              <td style="vertical-align:middle">
