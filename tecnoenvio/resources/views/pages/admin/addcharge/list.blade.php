...
@section('body')
  ...
  <div class="panel panel-default" id="pnlin">
    <div class="panel-body">
      <table class="table table-striped table-hover" id="dtble_addcharge">
        <thead>
          <tr>
        ...
        <tbody style="text-align: center;">
          @foreach ($addcharges as $addcharge)
            <tr item="{{$addcharge}}">
              <td>{{$addcharge->code}}</td>
              <td>{{$addcharge->name}}</td>
              <td>{{$addcharge->description}}</td>
              <td>{{$addcharge->value}}</td>
              <td>
                <ul class="table-actions">
                  <li><a onclick="icsAddChargesDelete($(this))" ><i class="fa fa-trash"  title="{{trans('messages.delete')}}"></i></a></li>
