    ...
    <form role="form" action="" method="" id ="formSerial">
      ...
      <fieldset class="form">
        ...
        <div class="form-group row @include('errors.field-class', ['field' => 'name'])">
          ...
          <div class="col-lg-9">
            <input class="form-control" placeholder="{{trans('user.name')}}" name="name" type="text" autofocus maxlength="100" min="5" required="true" value="{{isset($user) ? ucwords($user->name) : Input::get('name')}}" @include('form.readonly')>
        ...
        <div class="form-group row @include('errors.field-class', ['field' => 'last_name'])">
          ...
          <div class="col-lg-9">
            <input class="form-control" placeholder="{{trans('messages.last_name')}}" name="last_name" type="text" autofocus maxlength="100" min="5" required="true" value="{{isset($user) ? ucwords($user->last_name) : Input::get('last_name')}}" @include('form.readonly')>
        ...
        <div class="form-group row @include('errors.field-class', ['field' => 'country'])">
          ...
          <div class="col-lg-9">
            @if(!isset($readonly) || $readonly == false)
              <select style="width:100%;" class="form-control" name="country" placeholder="{{trans('messages.country')}}" required="true" value="{{isset($user) ? $user->country : Input::get('country')}}" @include('form.readonly')>
                @if(isset($countrys))
        ...
        <div class="form-group row @include('errors.field-class', ['field' => 'alt_email'])">
          ...
          <div class="col-lg-9">
            <input class="form-control" placeholder="{{trans('Correo alternativo')}}" name="alt_email" type="email" maxlength="50" min="5" value="{{isset($user) ? $user->alt_email : Input::get('alt_email')}}" @include('form.readonly')>
            @include('errors.field', ['field' => 'alt_email'])
          </div>
        </div>
        {{--id de la compañia--}}
        <input type="hidden" name="company" value="{{isset($company->id) ? $company->id : ''}}">
      </fieldset>
    </form>
  </div>
</div>
