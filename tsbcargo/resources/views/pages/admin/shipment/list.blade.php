      ...
      <table class="table table-striped table-hover table-responsive text-center" id="dtble3">
        <thead>
          <tr>
            ...
            <th>{{ucwords(trans('shipment.guide'))}}</th>
                   ...
                   <li>
                      <div class="dropdown" >
                        ...
                        <ul class="dropdown-menu ics_dropdown_menu_action" id="ics_unordenated_list">
                          ...
                          <li><a {{$value->transport == '1' ? "href=admin/shipments/{$value->id}/masterbill" : "href=admin/shipments/airwaybill/{$value->id}" }} target="_blank" >{{$value->transport == '1' ? trans('shipment.masterbill') : 'AWB'}}</a></li>
                          <li><a href="{{asset("admin/shipments/{$value->id}/cargomanifest")}}" target="_blank">{{trans('shipment.cargomanifest')}}</a></li>
                          <li><a href="{{asset("admin/shipments/{$value->id}/loadguide")}}" target="_blank">{{trans('Guia de carga')}}</a></li>
                        </ul>
                      </div>
                  </li>
                </ul>
