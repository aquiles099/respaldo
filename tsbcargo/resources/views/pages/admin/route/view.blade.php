<?php  use App\Models\Admin\Transport;
  $lang = App::getLocale();
?>
...
<form class="form" action="" method="post" id="formSerial">
  ...
  <fieldset class="form">
    ...
    <div class="form-group row @include('errors.field-class', ['field' => 'name'])">
      ...
      <div class="col-lg-10">
        <input class="form-control" placeholder="{{trans('route.name')}}" name="name" type="text" required="true" value="{{isset($route) ? ucwords($route->name) : Input::get('name')}}" @include('form.readonly')>
    ...
    <div class="form-group row @include('errors.field-class', ['field' => 'transport'])">
      ...
      <div class="col-lg-10">
        ...
        @else
        <?php
          $tipo = Transport::find($route->transport);
         ?>
        <input class="form-control" placeholder="" name="" type="text" required="true" value="{{isset($tipo) ? ucwords(($lang == 'es') ? $tipo->spanish : $tipo->english) : Input::get('transport')}}" @include('form.readonly')>
        @endif
    ...
    <div class="form-group row">
      <div class=" @include('errors.field-class', ['field' => 'origin_country'])">
        <label for="origin_country" class="col-lg-2 control-label">{{trans('route.originCountry')}}</label>
          <div class="col-lg-4">
            @if(!isset($readonly) || $readonly == false)
            <select style="width:100%;" class="form-control" id="ics_route_origin_country" name="origin_country" required="true" value="{{isset($route) ? ucwords($route->origin_country) : Input::get('origin_country')}}" @include('form.readonly') >
                <option value="0">{{trans('route.selectOption')}}</option>
            ...
            @else
            <input class="form-control" placeholder="" name="" type="text" required="true" value="{{isset($route) ? ucwords($route->getOriginCountry->name) : Input::get('origin_country')}}" @include('form.readonly')>
            @endif
      ...
      <div class=" @include('errors.field-class', ['field' => 'origin_city'])">
        ...
        <div class="col-lg-4">
          @if(!isset($readonly) || $readonly == false)
          <select style="width:100%;" class="form-control" id="ics_route_origin_city" name="origin_city" required="true" value="{{isset($route) ? ucwords($route->origin_city) : Input::get('origin_city')}}" @include('form.readonly') >
              <option value="0">{{trans('route.selectOption')}}</option>
              @if(isset($view))
          ...
          @else
          <input class="form-control" placeholder="" name="" type="text" required="true" value="{{isset($route) ? ucwords($route->getOriginCity->name) : Input::get('origin_city')}}" @include('form.readonly')>
          @endif
    ...
    <div class="form-group row">
      <div class=" @include('errors.field-class', ['field' => 'destiny_country'])">
        <label for="destiny_country" class="col-lg-2 control-label">{{trans('route.destinyCountry')}}</label>
          <div class="col-lg-4">
            @if(!isset($readonly) || $readonly == false)
            <select style="width:100%;" class="form-control" id="ics_route_destiny_country" name="destiny_country" required="true" value="{{isset($route) ? ucwords($route->destiny_country) : Input::get('origin_country')}}" @include('form.readonly') >
                <option value="0">{{trans('route.selectOption')}}</option>
                @foreach ($country as $key => $value)
            ...
            @else
            <input class="form-control" placeholder="" name="" type="text" required="true" value="{{isset($route) ? ucwords($route->getDestinyCountry->name) : Input::get('destiny_country')}}" @include('form.readonly')>
            @endif
            @include('errors.field', ['field' => 'origin_country'])
          </div>
      </div>
      <div class=" @include('errors.field-class', ['field' => 'destiny_city'])">
        ...
        <div class="col-lg-4">
          @if(!isset($readonly) || $readonly == false)
          <select style="width:100%;" class="form-control" id="ics_route_destiny_city" name="destiny_city" required="true" value="{{isset($route) ? ucwords($route->destiny_city ): Input::get('destiny_city')}}" @include('form.readonly') >
              <option value="0">{{trans('route.selectOption')}}</option>
              @if(isset($view))
          ...
          @else
          <input class="form-control" placeholder="" name="" type="text" required="true" value="{{isset($route) ? ucwords($route->getDestinyCity->name) : Input::get('destiny_city')}}" @include('form.readonly')>
          @endif
    ...
    <div class="form-group row @include('errors.field-class', ['field' => 'description'])">
      ...
      <div class="col-lg-10">
        <textarea class="form-control" placeholder="{{trans('route.description')}}" name="description" required="true"  @include('form.readonly')>{{isset($route) ? ucwords($route->description) : Input::get('description')}}</textarea>
