...
<div class="panel panel-default">
  <div class="panel-body">
        ...
        <div class="panel panel-default">
          <div class="panel-body">
            <table class="table table-striped">
              ...
              <tr>
                <td><b>{{trans('package.contact')}}:</b></td>
                <td>{{isset($package) ? (isset($package->to_client) ? $package->getToClient['email'] : $package->getToUser['email']) : ''}}</td>
              </tr>
            </table>
          </div>
        </div>
          ...
          <div class="panel-body">
            <table class="table table-striped">
              <tr>
                ...
                <td>{{isset($package->type) ? ($lang == 'es') ? ucwords($package->getType->spanish) : ucwords($package->getType->english) : Input::get('category')}}</td>
              </tr>
              <tr>
    ...
    <!-- {{'last '.$package->last_event.' events: '.$events_num}} -->
           ...
           <table class="table text-center table-striped table-responsive table-hover table-bordered" id="tableHeader">
            <thead>
              <tr style="padding: 4px;">
                ...
                <th style="width:20%;padding: 4px;">Descripción</th>
            ...
            <tbody id="table">
              ...
              <tr style="padding: 4px;">
                ...
                <td style="text-align: center;padding: 4px;"> {{$row->weight}}{{($package->unidad == 1) ? 'kg' : 'lb'}}</td>
                @if($package->type == 1)
                  <td style="text-align: center;padding: 4px;"> {{$row->volumetricweightm}} {{($package->unidad == 1) ? 'm3' : 'ft3'}}</td>
                @else
                  <td style="text-align: center;padding: 4px;"> {{$row->volumetricweighta}}{{($package->unidad == 1) ? 'Vkg' : 'Vlb'}}</td>
                @endif
              </tr>
               @endforeach
              </tbody>
            ...
            <tbody id="table">
              @if(isset($packageLog))
                @foreach ($packageLog as $row)
                  <tr style="padding: 4px;">
                    <td style="text-align: center;padding: 4px;" >{{($row->updated_at->format('M/d/Y h:i:s'))}}</td>
                    <td style="text-align: center;padding: 4px;" >{{ucwords($row->getUser['name'].' '.$row->getUser['last_name'])}}</td>
