...
@section('pageTitle', trans('clientAttention.searchServices'))
@section('title', trans('clientAttention.searchServices'))
@extends('pages.page')
      ...
      <table class="table table-striped table-hover" id="dtble_serv">
        ...
        <tbody>
          @foreach ($services as $service)
            <tr>
              <td>{{$service->id}}</td>
              <td><a class="infoRd" href="javascript:detailsservice('{{$service->id}}')">{{($lang == 'es') ? ucwords($service->spanish) : ucwords($service->english)}}</a></td>
              <td>{{$service->price}}</td>
            </tr>
          @endforeach
        </tbody>
