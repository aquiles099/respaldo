...
var editClient = function (id, user)
{
  ...
  $('#tltgnif').html(edit); /** modifca el titulo del modal **/
  if(user == true)
  {
    ...
    $('#load').load(path , function ()
    {
        ...
        $('#pnlft').append('<div class="panel-footer" id="pft"><a href="javascript:loadEditview(3 , '+id+')"><span class="badge" title="'+edit+'"><span class="glyphicon glyphicon-pencil"></span></span></a></div><input type="hidden" id="cpldcl" value = "1"><input type = "hidden" id="edtcl" value = "true">');
        $('#bckic').show();
    });
  }
  else /**  **/
  {
    ...
    $('#load').load(path + "/" + company + "/clients/" + id + "/read", function ()
      {
        $('#load').removeClass('cl');
        $('#pnlft').append('<div class="panel-footer" id="pft"><a href="javascript:loadEditview('+ sw +' , '+id+')"><span class="badge" title="'+edit+'"><span class="glyphicon glyphicon-pencil"></span></span></a></div><input type="hidden" id="cpldcl" value = '+ company +'><input type = "hidden" id="edtcl" value = "true">');
        $('#tltgnif').html(edit+' '+$('#'+id).text());
        $('#bckic').show();
        $('#pnldel').hide();
      });
  }
