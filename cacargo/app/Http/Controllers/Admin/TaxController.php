...
class TaxController extends Controller {
  ...
  public function details(Request $request, $id, $readonly = false)
  {
    ...
    {
      //dd($office);
      return view('pages.admin.tax.edit', $vars);
    }
    //Actualizar el impuesto
    $validator = $this->validateData($request);
    if (!is_null($validator)) {
      if ($validator->fails()) {
        return view('pages.admin.tax.edit', $vars)->withErrors($validator)
          ->with('errorMessage', trans('messages.checkRedFields'));
      }
    }
    $tax->update($validator->getData());
    $tax->save();
    return view('pages.admin.tax.edit', $vars)->with('successMessage', trans('tax.updated', [
      'name' => $tax->name,
      'code' => $tax->id
    ]));
  }
