...
class OperatorController extends Controller {
  ...
  public function details(Request $request, $id, $readonly = false) {
    ...
    if($this->isGET($request)) {
       return view('pages.admin.operator.edit', $vars);
     }
    $validator = $this->validateData($request, true, $id);
    if (!is_null($validator)) {
      if ($validator->fails()) {
        return view('pages.admin.operator.edit', $vars)->withErrors($validator)
    ...
    */
    ...
    $operator->save();
    return view('pages.admin.operator.edit', $vars)->with('successMessage', trans('operator.updated', [
      'name' => $operator->name,
    ]));
  }
  /**
