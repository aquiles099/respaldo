            ...
            <div class="panel panel-default">
              ...
              <div class="panel-body" style="margin-bottom: 24px">
                <fieldset>
                  ...
                  <!--Type of packages-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'type_packages'])">
                    <label class="col-lg-3 control-label" for="postal_code" id="label">{{trans('messages.type_packages')}}</label>
                    <div class="col-lg-9">
                      <select id="type_packages" class="form-control" name="type_packages">
                        <option {{($user->type_packages == 0) ? 'selected': ''}} value="0">{{trans('messages.optionSelect')}}</option>
                        <option {{($user->type_packages == '1') ? 'selected': ''}} value="1">{{trans('messages.banca')}}</option>
                        <option {{($user->type_packages == 2) ? 'selected': ''}} value="2">{{trans('messages.magazzine')}}</option>
                        <option {{($user->type_packages == 3) ? 'selected': ''}} value="3">{{trans('messages.various')}}</option>
                      </select>
                        @include('sections.errors', ['errors' =>  $errors, 'name' => 'postal_code'])
                    </div>
                  </div>
                  <!--Type of packages-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'postal_code'])">
                    <label class="col-lg-3 control-label" for="postal_code" id="label_specific">{{trans('messages.specific_packages')}}</label>
                    <div class="col-lg-9">
                      <input class="form-control" placeholder="{{trans('messages.specific_packages')}}" id="specific_packages" name="specific_packages" type="text" autofocus value="{{$user->specific_packages}}">
                      @include('sections.errors', ['errors' =>  $errors, 'name' => 'postal_code'])
                    </div>
                  </div>
                </fieldset>
