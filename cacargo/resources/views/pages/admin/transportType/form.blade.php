<form onsubmit="createLoad()" class="form" action="{{asset($path)}}" method="post">
  ...
  <fieldset class="form">
    ...
    <!--Precio-->
    <div class="form-group row @include('errors.field-class', ['field' => 'name'])">
      <label class="col-lg-3 control-label">{{trans('transportType.price')}}</label>
      <div class="col-lg-9">
        <input class="form-control" placeholder="{{trans('transportType.price')}}" name="price" type="text" required="true" value="{{isset($transport_type) ? $transport_type->price : Input::get('price')}}" @include('form.readonly')>
