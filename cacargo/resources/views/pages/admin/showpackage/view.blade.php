        ...
        <div class="panel-body">
          <table class="table table-striped">
            ...
            <tr>
              ...
              <td>{{isset($package) ? (isset($package->to_client) ? strtoupper($companyclient->name) : strtoupper($package->getCourier['name'])) : ''}}</td>
            </tr>
            <tr>
        ...
        <div class="panel-body">
          <table class="table table-striped">
            ...
            <tr>
              ...
              <td>{{isset($package) ? ucfirst($package->getLastEvent->name) : ''}}</td>
            </tr>
            <tr>
              ...
              <div class="col-lg-9">
                <select  class="form-control " id="event" name="event" required="true" @include('form.readonly')>
                     @foreach ($event as $row)
                        @if(($row->id > $package->last_event)&&($row->active == 1))
            ...
            <tbody id="table">
              @if(isset($packageLog))
                @foreach ($packageLog as $row)
                  <tr style="text-align: center">
                    <td>{{$package->code}}</td>
                    <td>{{ucwords($row->getUser['name'])}}</td>
                    <td>{{ucfirst($row->getEvent['description'])}}</td>
