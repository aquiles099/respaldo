...
<div class="navbar-default sidebar" role="navigation">
  <div id="menu" class="sidebar-nav navbar-collapse">
    <ul class="nav" id="side-menu">
      <?php $i=4; ?>
      <?php foreach(filterItems($tree) as $key => $item): ?>
       <?php if(existaccess($accesosresult,$item['node']['accesss'])): ?>
        <?php if(!isset($item['node']['url'])): ?>
          <?php if(isset($item['children']) && count($item['children']) > 0): ?>
            <li id="<?php echo e('step'.$i); ?>">
              <a id="item" href="#" class=""><i class="fa <?php echo e($item['node']['icon']); ?> fa-fw"></i> <?php echo e($item['node']['text']); ?> <span class="fa arrow"></span></a>
              <ul class="nav nav-second-level">
                <?php foreach($item['children'] as $subitem): ?>
                    <li><a id="subitem" href="<?php echo e($subitem['node']['url']); ?>"<?php if($subitem['node']['id'] == '14' || $subitem['node']['id'] == '15'): ?> target="_blank" <?php endif; ?>><i class="fa <?php echo e($subitem['node']['icon']); ?> fa-fw"></i> <?php echo e($subitem['node']['text']); ?></a></li>
                <?php endforeach; ?>
              </ul>
            </li>
          <?php endif; ?>
        <?php else: ?>
          <li id="<?php echo e('step'.$i); ?>"><a href="<?php echo e($item['node']['url']); ?>"><i class="fa <?php echo e($item['node']['icon']); ?> fa-fw"></i> <?php echo e($item['node']['text']); ?></a></li>
        <?php endif; ?>
      <?php endif; ?>
      <?php $i++; ?>
      <?php endforeach; ?>
