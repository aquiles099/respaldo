 ...
 *
 */
var icsAddChargesDelete = function (element, from) {
  try {
    var charge = getItem(element) || getItemFromParent(element);
    if(charge != undefined) {
      bootbox.confirm(eval(msg(messages.delete)), function(result) {
        if(result) {
          doForm(`./admin/addcharge/${charge.id}`, 'delete', undefined, from === undefined ? true : from);
        }
      });
    }
  } catch (e) {
