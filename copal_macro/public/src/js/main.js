...
var generalLoad = function () {
  $("#pnlin").append("<div id = 'load' class = 'load '><p class='ics-load-onModal'><i class='fa fa-spin fa-spinner'></i> Cargando...</p><input id='reload' type='hidden' value='false'>");
}
          ...
          message: $('#load').load(path + "/" + id + "/read", function () {
            ...
            */
            if($('#pnlin').hasClass( "cp" )) {
              ...
              $('#pnlft').append('<div class="panel-footer" id="pft"><a href="javascript:loadEditview('+ sw +' , '+id+')"><span class="badge" data-toggle="tooltip" title="Editar"><span class="glyphicon glyphicon-pencil"></span></span></a>&nbsp<a href="javascript:addClient('+ 2 +','+ id +', '+ nuser +')" id="adcl"><span class="badge" data-toggle="tooltip" title="Nuevo Cliente"><span class="glyphicon glyphicon-plus"></span></span></a></div>');
              $('#adcl').show();
            }
...
var evalJson = function (json)
{
  ...
  */
  if (!$('#error').length)
  {
    $('#load').append('<div class = "alert alert-danger"  id="ics-alert-danger"><b>Se Encontraron los siguientes errores: </b><button class = "close" data-dismiss = "alert"><span>&times;</span></button><ol id= "error"></ol></div>');
    /**
    * loop for show error
    */
...
var chartNums = function (a, b, c, d, e, f)
{
  ...
  var options =
  {
      title: "Cantidad de Paquetes en Fecha Establecida",
...
var showAlertInLoadBack = function (alert)
{
  if (alert == true)
  {
    $('#tltgnif').append('<div id="success" style="height: 45px"><div><div class = "alert alert-success"><b>procesado de manera correcta</b> <span><i class="fa fa-check-circle-o" aria-hidden="true"></i></span></div></div>');
    setTimeout(function()
    {
