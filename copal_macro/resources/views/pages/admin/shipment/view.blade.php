          ...
          <div class="panel-body">
            <div class="row">
              ...
              <div class="col-md-4" style="padding-right: 0px;padding-left: 0px;">
                ...
                <div class="col-lg-9">
                  <select class="form-control" name="icsBookigStatus" id="ics_shipment_status">
                    @foreach($event as $key => $value)
                        @if($value->id > $shipment->last_event)
                          <option item ="{{$value->toInnerJson()}}" name="{{$value->name}}" value="{{$value->id}}">{{$value->name}}</option>
                        @endif
                    @endforeach
                  </select>
                </div>
              ...
              <tbody>
                @foreach($shipmentDetail as $key => $value)
                  <tr item="{{$value->toInnerJson()}}">
                    ...
                    <td style="padding:4px">{{$value->volume}}</td>
                    <td style="padding:4px">{{$value->weight}}</td>
                  </tr>
                @endforeach
              </tbody>
            </table>
