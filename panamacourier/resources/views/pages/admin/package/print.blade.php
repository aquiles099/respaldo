          ...
          <div class="row">
            <div class="col-md-12">
              <table class="text-center" style="width:100%">
                ...
                <tbody>
                  @foreach($detailspack as $pack)
                  <tr>
                    <td>{{(isset($package)) ? $package->created_at->format('Y-m-d') : '' }}</td>
                    <td>{{(isset($pack)) ? $pack->weight : '' }} lb</td>
                    <td>{{(isset($pack)) ? $pack->pieces : '' }} </td>
                    <td>{{(isset($pack)) ? $pack->description : '' }}</td>
                    <td>@if (isset($pack)) {{$pack->width}}x{{$pack->height}}x{{$pack->large}}@endif</td>
                  </tr>
                  @endforeach
                </tbody>
              </table>
