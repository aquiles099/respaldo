      ...
      <div class="col-lg-9">
        <select style="width:100%;" class="form-control" id="transport" name="transport" required="true" @include('form.readonly')>
            @foreach ($transports as $transport)
              ...
              <option {{(isset($promotions) ? $promotions->transport : Input::get('transport')) == $option['id'] ? 'selected' : ''}} item="{{$transport->toInnerJson()}}" value="{{$option['id']}}">{{ucwords($option['text'])}}</option>
            @endforeach
        </select>
