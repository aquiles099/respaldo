  ...
  <div class="panel-body">
          ...
          <div class="panel-body">
            <table class="table table-striped">
              ...
              <tr>
                ...
                <td>{{isset($package) ? $package->address_shipper.', '.$package->city_shipper.', '.$package->region_shipper.', '.$package->country_shipper : Input::get('category')}}</td>
              </tr>
              <tr>
                <td><b>{{trans('package.to')}}:</b></td>
                @if($package->to_user == 0)
                  <td>{{isset($package->destin_name) ? $package->destin_name : ''}}</td>
                @endif
                @if($package->to_user != 0)
                  <td>
                    @if (isset($package))
                      @if (isset($package))
                        {{$package->getToClient['code'] . "-" . $package->getToClient['name']}}
                      @else
                        {{ $package->getToUser['code'] . "-" . $package->getToUser['name'] . " " . $package->getToUser['last_name'] }}
                      @endif
                    @else
        ...
        <div class="panel panel-default">
          <div class="panel-body">
            <table class="table table-striped">
              ...
              <tr>
                <td>
                  <b>{{trans('pickup.destination')}}:</b>
                </td>
                <?php $country = Country::find($package->country_consig); ?>
                <td>
                  @if (isset($package))
                    {{isset ($package->address_consig) ? $package->address_consig : trans('messages.unknown') }}, {{isset($package->city_consig) ? $package->city_consig : trans('messages.unknown')}}, {{ isset ($package->region_consig) ? $package->region_consig : trans('messages.unknown') }}, {{ isset ($country->name) ? $country->name : trans('messages.unknown')}}
                  @else
                    {{trans('messages.unknown')}}
                  @endif
               </td>
              ...
              <tr>
                <td><b>{{trans('package.phone')}}:</b></td>
                <td>{{isset($package) ? ucfirst($package->destin_phone): Input::get('category')}}</td>
              </tr>
            </table>
          </div>
        </div>
    ...
    @if(Session::get('key-sesion')['type'] == HUserType::OPERATOR)
      @if(!isset($noInvoice) || $noInvoice != true)
        @if($package->last_event < $events_num)
          <div class="row">
              ...
              <div class="panel panel-default">
                <div class="panel-body">
                  ...
                  <div class="col-md-4 " style="padding-right: 0px;padding-left: 0px;">
                    ...
                    <div class="col-lg-9">
                      <select style="width:100%;"  class="form-control " id="event" name="event" required="true" @include('form.readonly')>
                  ...
                  <div class="col-md-2 " style="padding-right: 0px;padding-left: 0px;">
                    <button  type="button" class="btn btn-primary" onclick="changestatuspickup({{$package->id}})">
           ...
           <table class="table text-center table-striped table-responsive table-hover table-bordered" id="tableHeader">
            ...
            <tbody id="table">
              ...
              <tr style="padding: 4px;">
                ...
                <td style="text-align: center;padding: 4px;"> {{$row->description}}</td>
                <td style="text-align: center;padding: 4px;"> {{$row->weight}}lb</td>
                <td style="text-align: center;padding: 4px;"> {{$row->volumetricweight }}ft<sup>3</sup></td>
              </tr>
               @endforeach
              </tbody>
            </table>
        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <table class="table text-center table-striped table-responsive table-hover table-bordered" id="tableHeader">
              <thead>
                <tr style="padding: 4px;">
                  <th style="width:20%">{{trans('Fecha')}}</th>
              ...
              <tbody id="table">
                @if(isset($packageLog))
                  @foreach ($packageLog as $row)
                    <tr style="padding: 4px;">
                      <td style="text-align: center; padding: 4px;" >{{($package->updated_at->format('M/d/Y h:i:s'))}}</td>
