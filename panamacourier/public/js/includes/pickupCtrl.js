...
function setDataDestinationUser(destinationData)
{
  if (destinationData)
  {
    var sele = document.getElementById("destin_country");
    var option = sele.children[2];
    for (var i = 0; i < sele.children.length; i++) {
      if(sele.children[i].text == destinationData.country){
        $('#destin_country').prop('selectedIndex',i);
        $('#destin_country').select2();
      }
    }
     ...
     " <div class='dimensmedidas'  id='divlarge' >"+"\n"+
        ...
        "<div class='dimensmedidas'  id='divlarge' >"+"\n"+
          "<div class='row' style='margin-right: 0px;margin-left: 0px;'>"+"\n"+
            "<label class='col-lg-2 control-label' id='typeLabel' style='line-height: 14px;'>"+text.typepickup+"</label>"+"\n"+
            "<div class='col-lg-10' style='padding-bottom: 5px;'>"+"\n"+
              "<select style='width:100%;' class='form-control' placeholder='"+text.typepickup+"' id='typepickup"+aux+"' name='typepickup"+aux+"' maxlength='10' min='1' required='true' value='' >"+"\n"+
              "<option value=''>"+text.chooseOption+"</option>"+"\n"+
              " <select/>"+"\n"+
            "</div>"+"\n"+
          " </div>"+"\n"+
        " </div>"+"\n"+
        "<div class='hidden dimensmedidas'  id='divlarge' >"+"\n"+
          "<div class='row' style='margin-right: 0px;margin-left: 0px;'>"+"\n"+
            "<label class='col-lg-2 control-label' id='typeLabel' style='line-height: 12px;'>"+text.numberpart+"</label>"+"\n"+
            "<div class='col-lg-10' style='padding-bottom: 5px;'>"+"\n"+
              "<select style='width:100%;' class='form-control' placeholder='"+text.numberpart+"' id='numberparts"+aux+"' onchange=valselect('"+aux+"') name='numberparts"+aux+"' maxlength='10' min='1' required='true' value='' >"+"\n"+
              " <select/>"+"\n"+
            "</div>"+"\n"+
          "</div>"+"\n"+
        "</div>"+"\n"+
        ...
        "<div class='dimensmedidas'  id='divlarge'>"+"\n"+
          "<label class='col-lg-2 control-label' id='typeLabel' >"+text.invoice+"</label>"+"\n"+
          "<div class='col-lg-10'>"+"\n"+
            "<input type='number' class='form-control' placeholder='"+text.invoice+"' id='invoiced"+aux+"' name='invoiced"+aux+"' maxlength='10' min='1' required='true' value=''>"+"\n"+
            " <span class='help-block' style='margin-bottom: 0px;'></span>"+"\n"+
          "</div>"+"\n"+
        "</div>"+"\n"+

        "<div class='dimensmedidas'  id='divlarge'>"+"\n"+
          "<label class='col-lg-2 control-label' id='typeLabel' >"+text.tracking+"</label>"+"\n"+
          "<div class='col-lg-10'>"+"\n"+
            "<input type='number' class='form-control' placeholder='"+text.tracking+"' id='tracking"+aux+"' name='tracking"+aux+"' maxlength='10' min='1' required='true' value=''>"+"\n"+
            " <span class='help-block' style='margin-bottom: 0px;'></span>"+"\n"+
          "</div>"+"\n"+
        "</div>"+"\n"+
    "</div>"+"\n"+
...
*/
var AddType = function (value, type)
{
  ...
  $.ajax({
    ...
    {
      LoadDinamicSelectType(json.message, value, type);
    },
    error: function (e)
    {
      log(e);
...
var LoadDinamicSelectType = function (json, value, type)
{

  $.each(json, function(i, item)
  {
    $('#typepickup' + value).append('<option '+((json[i].id==type) ? 'selected' : '')+' value ="'+json[i].id+'">' + json[i].name.toUpperCase()  + '</option>');
  });
}

var AddTypeSelect = function ()
{
  var url = `${window.location.origin}${window.location.pathname}/typeselected`;
  $.ajax({
    url: url,
    type: 'GET',
    dataType: 'json',
    ...
    success: function (json)
    {
      selectDinamicSelectType(json.message);
    },
    error: function (e)
    {
      log(e);
...
var selectDinamicSelectType = function (json)
{
  $.each(json, function(i, item)
  {
    $('#volumetricweightm'+(i+1)).val(json[i].volumetricweightm);
    $('#volumetricweighta'+(i+1)).val(json[i].volumetricweighta);
    $('#typepickup'+(i+1)).val(parseInt(json[i].type)).change();
    $('#typepickup'+(i+1)).select2();
  });
}
...
function pickupDelete(element, from)
{
  var text = translate();
  try
  {
    ...
    if(pickup != undefined)
    {
      bootbox.confirm(text.deletepickup, function(result)
      {
        if(result)
        {
          doForm(`./admin/pickup/`+element, 'delete', undefined, from === undefined ? true : from);
        }
      });
    }
  }
...
function pickupadd(id)
{
  ...
  var text = translate();
  bootbox.confirm(text.confirmToWarehouse,
    function(result)
    {
       $.ajax({
        url: url,
      ...
      $('#contentpack').append("<div class='row tab-pane fade' id='pick"+(i+1)+"' style='padding:20px'>"+"\n"+
        "<div class='col-md-8'>"+"\n"+
         " <div class='dimensmedidas'  id='divlarge' >"+"\n"+
            ...
            "<div class='dimensmedidas'  id='divlarge' >"+"\n"+
              "<div class='row' style='margin-right: 0px;margin-left: 0px;'>"+"\n"+
                "<label class='col-lg-2 control-label' id='typeLabel' style='line-height: 14px;'>Tipo de recolecta</label>"+"\n"+
                "<div class='col-lg-10' style='padding-bottom: 5px;'>"+"\n"+
                  "<select style='width:100%' class='form-control' placeholder='Tipo' id='typepickup"+(i+1)+"' name='typepickup"+(i+1)+"' maxlength='10' min='1' required='true' value='' >"+"\n"+
                  "<option value=''>Seleccione Tipo de Recolecta</option>"+"\n"+
                  " <select/>"+"\n"+
                "</div>"+"\n"+
              " </div>"+"\n"+
            " </div>"+"\n"+
            "<div class='hidden dimensmedidas'  id='divlarge' >"+"\n"+
              "<div class='row' style='margin-right: 0px;margin-left: 0px;'>"+"\n"+
                ...
                "<div class='col-lg-10' style='padding-bottom: 5px;'>"+"\n"+
                  "<select style='width:100%;' class='form-control' placeholder='Partes' id='numberparts"+(i+1)+"' onchange=valselect('"+(i+1)+"') name='numberparts"+(i+1)+"' maxlength='10' min='1' required='true' value='"+json[i].description+"' >"+"\n"+
                  " <select/>"+"\n"+
                "</div>"+"\n"+
              "</div>"+"\n"+
            "</div>"+"\n"+
        ...
        "</div>"+"\n"+
         ...
         "</div>"+"\n"+
         " </div>");
         console.log('tipo: '+json[i].type);
         AddType((i+1),json[i].type);
