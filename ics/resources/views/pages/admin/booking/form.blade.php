...
<form role="form" action="{{asset($path)}}" onsubmit="createLoad()" method="post" id="ics_cargo_booking_form" novalidate enctype="multipart/form-data" >
  ...
  <fieldset>
    ...
    <ul class="nav nav-tabs nav-justified">
      ...
      <li id="ics_inactive_tab4" class="ics-inactive-tab"><a data-toggle = "tab" href="#ics_tab_menu4">{{trans('booking.notes')}} <span><i class="fa fa-plus-square-o" aria-hidden="true"></i></span></a></li>
    </ul>
        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
                ...
                <select class="form-control" id="transport" name="transport" required="true" @include('form.readonly')>
                  @if(isset($transports))
                    @foreach($transports as $key => $transport)
                      <option {{isset($booking) ? ($booking->transport == $transport->id) ? 'selected' : '' : '' }} name ="{{($lang == 'es') ? ucwords($transport->spanish) : ucwords($transport->english)}}" value="{{$transport->id}}">{{($lang == 'es') ? ucwords($transport->spanish) : ucwords($transport->english)}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
            ...
            <div class="form-group row @include('errors.field-class', ['field' => 'from'])" id="type" >
              ...
              <div class="col-lg-4">
                <select class="form-control" id="transport" name="from_country" required="true" @include('form.readonly')>
                  <option value="">{{trans('booking.selectOption')}}</option>
                  @if(isset($countrys))
            ...
            <div class="breadcrumb">
              <span>{{trans('booking.transport_info')}}</span>
            </div>
            <div class="form-group row @include('errors.field-class', ['field' => 'from'])" id="type" >
              <label class="col-lg-2 control-label" id="typeLabel" > {{trans('booking.transporter')}}</label>
              <div class="col-lg-4">
                <select class="form-control" id="transporter" name="transporter" required="true" @include('form.readonly')>
                  <option value="0">{{trans('booking.selectOption')}}</option>
                  @if(isset($transporters))
                    @foreach($transporters as $key => $transporter)
                      <option {{isset($booking->transporter) ? ($booking->transporter == $transporter->id) ? 'selected' : '' : '' }} name ="{{$transporter->code}}{{$transporter->name}}" value="{{$transporter->id}}">{{$transporter->code}} {{$transporter->name}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
              <label class="col-lg-2 control-label" id="typeLabel" >{{trans('booking.dangerous')}}</label>
              <div class="col-lg-4">
                <select class="form-control" id="dangerous" name="dangerous" required="true" @include('form.readonly')>
                    <option {{isset($booking) ? ($booking->dangerous == null) ? 'selected' : '' : '' }}value="0">{{trans('booking.selectOption')}}</option>
                    <option {{isset($booking->dangerous) ? ($booking->dangerous == 1) ? 'selected' : '' : '' }} value="1">{{trans('booking.yes')}}</option>
                    <option {{isset($booking->dangerous) ? ($booking->dangerous == 2) ? 'selected' : '' : '' }} value="2">{{trans('booking.no')}}</option>
                    <!--
                      <option {{isset($booking) ? ($booking->to_country == $country->id) ? 'selected' : '' : '' }} name ="{{$country->code}}{{$country->name}}" value="{{$country->id}}">{{$country->code}} {{$country->name}}</option>
                   -->
                </select>
            ...
            <!--BARCO/VIAJE-->
            <div class="form-group row @include('errors.field-class', ['field' => 'from'])" id="type" >
              <label class="col-lg-2 control-label" id="typeLabel" >{{trans('booking.travel')}}</label>
              <div class="col-lg-4">
                <input type="text" class="form-control" placeholder="{{trans('Nombre del Barco o Viaje')}}" id="vessel" name="vessel" type="text" min="1" required="true" value="{{isset($booking->vessel) ? $booking->vessel : ''}}" @include('form.readonly')>
              </div>
              <!--VIA-->
              <label class="col-lg-2 control-label" id="typeLabel" >{{trans('Via')}}</label>
              <div class="col-lg-4">
                <input type="text" class="form-control" placeholder="{{trans('Via')}}" id="way" name="way" type="text" min="1" required="true" value="{{isset($booking->way) ? $booking->way : ''}}" @include('form.readonly')>
              </div>
            </div>
            <!-- -->
            <div class="form-group row @include('errors.field-class', ['field' => 'from'])" id="type" >
              <label class="col-lg-2 control-label" id="typeLabel" >{{trans('booking.maritime_guide')}}</label>
              <div class="col-lg-4">
                  <input type="text" class="form-control" placeholder="{{trans('Guia Maritima')}}" id="guide" name="guide" type="text" min="1" required="true" value="{{isset($booking->guide) ? $booking->guide : ''}}" @include('form.readonly')>
              </div>
              <label class="col-lg-2 control-label" id="typeLabel" >{{trans('booking.employee')}}</label>
              <div class="col-lg-4">
                <select class="form-control" id="employee" name="employee" required="true" @include('form.readonly')>
                  <option value="0">{{trans('booking.selectOption')}}</option>
                  @if(isset($operators))
                    @foreach($operators as $key => $operator)
                      <option {{isset($booking->employee) ? ($booking->employee == $operator->id) ? 'selected' : '' : '' }} name ="{{$operator->code}}{{$operator->name}}" value="{{$operator->id}}">{{$operator->code}} {{$operator->name}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
      ...
      <div id="ics_tab_menu1" class="tab-pane fade" >
        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12">
                <label class="col-md-3" >{{trans('booking.reference')}}</label>
                <div class="col-md-9">
                  <input class="form-control" type="text" maxlength="30" placeholder="{{trans('booking.reference')}}" id="references" name="references" value="{{isset($booking) ? $booking->reference : Input::get('references')}}">
                </div>
              </div>
              <div class="col-md-12">
                <br>
              ...
              <div class="col-md-12">
                <label class="col-md-3" >{{trans('booking.cotiza')}}</label>
                <div class="col-md-9">
                  <input class="form-control" type="text" name="cotiza" placeholder="{{trans('booking.cotiza')}}" id="cotiza" maxlength="30" value="{{isset($booking) ? $booking->cotiza : Input::get('cotiza')}}">
                </div>
              </div>
              <div class="col-md-12">
                <!--divider-->
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-4">
                <!--Divider-->
                <div class="breadcrumb">
                  <span><center>{{trans('booking.shipperInformation')}}</center></span>
                </div>
                <!--Shipper Information-->
                <fieldset class="form">
                  <!--user-->
                      ...
                      <select class="form-control" id="shipperName" name="shipperName" required="true" @include('form.readonly')>
                        @if(isset($users))
                          ...
                          @foreach($users as $key => $user)
                            <option {{isset($booking->shipper) ? ($booking->shipper == $user->id) ? 'selected' : '' : '' }} name ="{{$user->name}} {{$user->last_name}}" item="{{$user->toInnerJson()}}" value="{{$user->id}}">{{$user->code}} {{$user->name}} {{$user->last_name}}</option>
                          @endforeach
                        @endif
                      </select>
                  ...
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientName')}}" id="cons_name" name="cons_name" type="text" maxlength="25" min="5"  value="{{isset($booking->shipper) ? $booking->getShipper['name'] : clear(Input::get('name'))}}" @include('form.readonly')>
                ...
                <div class="form-group row @include('errors.field-class', ['field' => 'name'])" id="clientName" >
                  ...
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientLastName')}}" id="cons_lastname" name="cons_lastname" type="text" maxlength="25" min="5"  value="{{isset($booking->shipper) ? $booking->getShipper['last_name'] : clear(Input::get('name'))}}" @include('form.readonly')>
                      @include('errors.field', ['field' => 'name'])
                  </div>
                </div>
                <!-- -->
                <div class="form-group row @include('errors.field-class', ['field' => 'dni'])">
                  ...
                  <div class="col-lg-9">
                    <input class="form-control" placeholder="{{trans('messages.dni')}}" name="cons_dni" id="cons_dni" type="text"  maxlength="100" min="5" readonly="true" required="true" value="{{isset($booking->shipper) ? $booking->getShipper['dni'] : Input::get('dni')}}" @include('form.readonly')>
                ...
                <div class="form-group row @include('errors.field-class', ['field' => 'email'])" id="clientEmail" >
                  ...
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientEmail')}}" id="cons_email" name="cons_email" type="text" maxlength="50" min="5"  value="{{isset($booking->shipper) ? $booking->getShipper['email'] : clear(Input::get('email'))}}"@include('form.readonly')>
              ...
              <div class="col-md-4">
                <!--Divider-->
                <div class="breadcrumb">
                  <span><center>{{trans('booking.consigneeInformation')}}</center></span>
                </div>
                <!--Shipper Information-->
                <fieldset class="form">
                  <!--user-->
                  ...
                  <label class="col-lg-3 control-label" id="labelName" >{{trans('booking.name')}}</label>
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientName')}}" id="destin_name" name="destin_name" type="text" maxlength="25" min="5"  value="{{isset($booking->consigneer) ? $booking->getConsigneer['name'] : clear(Input::get('name'))}}" @include('form.readonly')>
                      @include('errors.field', ['field' => 'name'])
                  </div>
                </div>
                 <!--last name-->
                <div class="form-group row @include('errors.field-class', ['field' => 'name'])" id="clientName" >
                  <label class="col-lg-3 control-label" id="labelName" >{{trans('package.clientLastName')}}</label>
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientLastName')}}" id="destin_lastname" name="destin_lastname" type="text" maxlength="25" min="5"  value="{{isset($booking->consigneer) ? $booking->getConsigneer['last_name'] : clear(Input::get('name'))}}" @include('form.readonly')>
                      @include('errors.field', ['field' => 'name'])
                  </div>
                </div>
                <!-- -->
                <div class="form-group row @include('errors.field-class', ['field' => 'dni'])">
                  <label class="col-lg-3 control-label">{{trans('messages.dni')}} </label>
                  <div class="col-lg-9">
                    <input class="form-control" placeholder="{{trans('messages.dni')}}" name="destin_dni" id="destin_dni" type="text"  maxlength="100" min="5" readonly="true" required="true" value="{{isset($booking->consigneer) ? $booking->getConsigneer['dni'] : Input::get('dni')}}" @include('form.readonly')>
                    @include('errors.field', ['field' => 'dni'])
                  </div>
                </div>
                <!--cell_phone-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'localPhone'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.phone')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="destin_phonedestin" name="destin_phone" value="{{isset($booking->getConsigneer) ? $booking->getConsigneer->celular : Input::get('shipperPhone')}}" placeholder="{{trans('booking.phone')}}"required="true" @include('form.readonly')>
                      @include('errors.field', ['field' => 'shipperPhone'])
                    </div>
                  </div>
                  <!--cell_phone-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperPhone'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('package.cell')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="destin_cell" name="destin_cell" value="{{isset($booking->getConsigneer) ? $booking->getConsigneer->celular : Input::get('shipperPhone')}}" placeholder="{{trans('package.cell')}}"required="true" @include('form.readonly')>
                      @include('errors.field', ['field' => 'shipperPhone'])
                    </div>
                  </div>
                   <!-- -->
                <div class="form-group row @include('errors.field-class', ['field' => 'email'])" id="clientEmail" >
                  <label class="col-lg-3 control-label" id="labelEmail" >{{trans('package.clientEmail')}}</label>
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientEmail')}}" id="destin_email" name="destin_email" type="text" maxlength="50" min="5"  value="{{isset($booking->consigneer) ? $booking->getConsigneer['email'] : clear(Input::get('email'))}}"@include('form.readonly')>
                      @include('errors.field', ['field' => 'email'])
                  </div>
                </div>
                  <!--country-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperCountry'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.country')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="destin_country" name="destin_country" value="{{isset($booking->getConsigneer) ? $booking->getConsigneer->country : Input::get('shipperCountry')}}" placeholder="{{trans('booking.country')}}"equired="true" @include('form.readonly')>
                      @include('errors.field', ['field' => 'shipperCountry'])
                    </div>
                  </div>
                  <!--region-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperRegion'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.region')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="destin_region" name="destin_region" value="{{isset($booking->getConsigneer) ? $booking->getConsigneer->region : Input::get('shipperRegion')}}" placeholder="{{trans('booking.region')}}"required="true" @include('form.readonly')>
                    @include('errors.field', ['field' => 'shipperRegion'])
                    </div>
                  </div>
                  <!--city-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperCity'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.city')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="destin_city" name="destin_city" value="{{isset($booking->getConsigneer) ? $booking->getConsigneer->city : Input::get('shipperCity')}}" placeholder="{{trans('booking.city')}}"required="true" @include('form.readonly')>
                    @include('errors.field', ['field' => 'shipperCity'])
                    </div>
                  </div>
                  <!--address-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperAdress'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.address')}}</label>
                    <div class="col-lg-9">
                        <textarea readonly="true" class="form-control" id="destin_direction" name="destin_direction" value="{{isset($booking->getConsigneer) ? $booking->getConsigneer->address : Input::get('shipperAdress')}}" placeholder="{{trans('booking.address')}}"required="true" @include('form.readonly')>{{isset($booking->getConsigneer) ? $booking->getConsigneer->address : Input::get('shipperAdress')}}</textarea>
                    @include('errors.field', ['field' => 'shipperAdress'])
                    </div>
                  </div>
                  <!--postal code-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperPostalCode'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.postalCode')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="destin_zipcode" name="destin_zipcode" value="{{isset($booking->getConsigneer) ? $booking->getConsigneer->postal_code : Input::get('shipperPostalCode')}}" placeholder="{{trans('booking.postalCode')}}"required="true" @include('form.readonly')>
                      @include('errors.field', ['field' => 'shipperPostalCode'])
                    </div>
                  </div>
                </fieldset>
              ...
              <div class="col-md-4">
                <!--Divider-->
                <div class="breadcrumb">
                  <span><center> {{trans('booking.agent_info')}}</center></span>
                </div>
                <!--Agent Information-->
                <fieldset class="form">
                  <!--user-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperName'])" >
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.name')}}</label>
                    <div class="col-lg-9">
                      <select class="form-control" id="agentName" name="agentName" required="true" @include('form.readonly')>
                        @if(isset($users))
                          <option value="">{{trans('booking.selectOption')}}</option>
                          @foreach($users as $key => $user)
                            <option {{isset($booking->agent) ? ($booking->agent == $user->id) ? 'selected' : '' : '' }} name ="{{$user->name}} {{$user->last_name}}" item="{{$user->toInnerJson()}}" value="{{$user->id}}">{{$user->code}} {{$user->name}} {{$user->last_name}}</option>
                          @endforeach
                        @endif
                      </select>
                        @include('errors.field', ['field' => 'shipperName'])
                  ...
                  <label class="col-lg-3 control-label" id="labelName" >{{trans('booking.name')}}</label>
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientName')}}" id="agent_name" name="agent_name" type="text" maxlength="25" min="5"  value="{{isset($booking->agent) ? $booking->getAgent['name'] : clear(Input::get('name'))}}" @include('form.readonly')>
                      @include('errors.field', ['field' => 'name'])
                  </div>
                </div>
                 <!--last name-->
                <div class="form-group row @include('errors.field-class', ['field' => 'name'])" id="clientName" >
                  <label class="col-lg-3 control-label" id="labelName" >{{trans('package.clientLastName')}}</label>
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientLastName')}}" id="agent_lastname" name="agent_lastname" type="text" maxlength="25" min="5"  value="{{isset($booking->agent) ? $booking->getAgent['last_name'] : clear(Input::get('name'))}}" @include('form.readonly')>
                      @include('errors.field', ['field' => 'name'])
                  </div>
                </div>
                <!-- -->
                <div class="form-group row @include('errors.field-class', ['field' => 'dni'])">
                  <label class="col-lg-3 control-label">{{trans('messages.dni')}} </label>
                  <div class="col-lg-9">
                    <input class="form-control" placeholder="{{trans('messages.dni')}}" name="agent_dni" id="agent_dni" type="text"  maxlength="100" min="5" readonly="true" required="true" value="{{isset($booking->agent) ? $booking->getAgent['dni'] : Input::get('dni')}}" @include('form.readonly')>
                    @include('errors.field', ['field' => 'dni'])
                  </div>
                </div>
                <!--cell_phone-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'localPhone'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.phone')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="agent_phone" name="agent_phone" value="{{isset($booking->getAgent) ? $booking->getAgent->celular : Input::get('shipperPhone')}}" placeholder="{{trans('booking.phone')}}"required="true" @include('form.readonly')>
                      @include('errors.field', ['field' => 'shipperPhone'])
                    </div>
                  </div>
                  <!--cell_phone-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperPhone'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('package.cell')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="agent_cell" name="agent_cell" value="{{isset($booking->getAgent) ? $booking->getAgent->celular : Input::get('shipperPhone')}}" placeholder="{{trans('package.cell')}}"required="true" @include('form.readonly')>
                      @include('errors.field', ['field' => 'shipperPhone'])
                    </div>
                  </div>
                   <!-- -->
                <div class="form-group row @include('errors.field-class', ['field' => 'email'])" id="clientEmail" >
                  <label class="col-lg-3 control-label" id="labelEmail" >{{trans('package.clientEmail')}}</label>
                  <div class="col-lg-9">
                    <input class="form-control" readonly="true" placeholder="{{trans('package.clientEmail')}}" id="agent_email" name="agent_email" type="text" maxlength="50" min="5"  value="{{isset($booking->agent) ? $booking->getAgent['email'] : clear(Input::get('email'))}}"@include('form.readonly')>
                      @include('errors.field', ['field' => 'email'])
                  </div>
                </div>
                  <!--country-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperCountry'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.country')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="agent_country" name="agent_country" value="{{isset($booking->getAgent) ? $booking->getAgent->country : Input::get('shipperCountry')}}" placeholder="{{trans('booking.country')}}"equired="true" @include('form.readonly')>
                      @include('errors.field', ['field' => 'shipperCountry'])
                    </div>
                  </div>
                  <!--region-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperRegion'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.region')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="agent_region" name="agent_region" value="{{isset($booking->getAgent) ? $booking->getAgent->region : Input::get('shipperRegion')}}" placeholder="{{trans('booking.region')}}"required="true" @include('form.readonly')>
                    @include('errors.field', ['field' => 'shipperRegion'])
                    </div>
                  </div>
                  <!--city-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperCity'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.city')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="agent_city" name="agent_city" value="{{isset($booking->getAgent) ? $booking->getAgent->city : Input::get('shipperCity')}}" placeholder="{{trans('booking.city')}}"required="true" @include('form.readonly')>
                    @include('errors.field', ['field' => 'shipperCity'])
                    </div>
                  </div>
                  <!--address-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperAdress'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.address')}}</label>
                    <div class="col-lg-9">
                        <textarea readonly="true" class="form-control" id="agent_direction" name="agent_direction" value="{{isset($booking->getAgent) ? $booking->getAgent->address : Input::get('shipperAdress')}}" placeholder="{{trans('booking.address')}}"required="true" @include('form.readonly')>{{isset($booking->getAgent) ? $booking->getAgent->address : Input::get('shipperAdress')}}</textarea>
                    @include('errors.field', ['field' => 'shipperAdress'])
                    </div>
                  </div>
                  <!--postal code-->
                  <div class="form-group row @include('errors.field-class', ['field' => 'shipperPostalCode'])">
                    <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.postalCode')}}</label>
                    <div class="col-lg-9">
                        <input type="text" readonly="true" class="form-control" id="agent_zipcode" name="agent_zipcode" value="{{isset($booking->getAgent) ? $booking->getAgent->postal_code : Input::get('shipperPostalCode')}}" placeholder="{{trans('booking.postalCode')}}"required="true" @include('form.readonly')>
                      @include('errors.field', ['field' => 'shipperPostalCode'])
                    </div>
                  </div>
                </fieldset>
        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            ...
            <div class="form-group row @include('errors.field-class', ['field' => 'aditionalInformation'])" id="type" >
              <label class="col-lg-3 control-label" id="typeLabel" >{{trans('booking.note')}}</label>
              <div class="col-lg-9">
                  <textarea class="form-control" id="aditionalInformation" name="aditional_information" value="{{Input::get('aditionalInformation')}}" placeholder="{{trans('booking.notes')}}"required="true" @include('form.readonly')>{{isset($booking) ? $booking->aditional_information : Input::get('aditionalInformation')}}</textarea>
