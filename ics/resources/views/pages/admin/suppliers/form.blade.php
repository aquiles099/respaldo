            ...
            <div class="row">
              <div class="col-md-12">
                ...
                <div class="form-group row @include('errors.field-class', ['field' => 'city'])">
                  ...
                  <div class="col-lg-10">
                    @if(!isset($readonly) || $readonly == false)
                    <select style="width:100%;" class="form-control" id="ics_vessel_city" name="city" required="true" value="{{isset($vessel) ? $route->city : Input::get('city')}}" @include('form.readonly') >
                      <option value="0">{{trans('route.selectOption')}}</option>
                      @if(isset($view))
                        @foreach($country as $key => $value)
              ...
              <div class="col-md-12">
                ...
                <div class="form-group row @include('errors.field-class', ['field' => 'country'])">
                  ...
                  <div class="col-lg-10">
                    @if(!isset($readonly) || $readonly == false)
                    <select style="width:100%;" class="form-control" id="ics_vessel_country2" name="country" required="true" value="{{isset($vessel) ? $route->country : Input::get('country')}}" @include('form.readonly') >
                      <option value="0">{{trans('route.selectOption')}}</option>
                      @foreach($country as $key => $value)
                        <option {{(isset($vessel)) && ($vessel->country == $value->id) ? 'selected' : Input::get('country') == $value->id ? 'selected' : '' }} item="{{$value->toInnerJson()}}" value="{{$value->id}}">{{ucwords($value->name)}}</option>
                      @endforeach
                ...
                <div class="form-group row @include('errors.field-class', ['field' => 'city'])">
                  ...
                  <div class="col-lg-10">
                    @if(!isset($readonly) || $readonly == false)
                    <select style="width:100%;" class="form-control" id="ics_vessel_city2" name="city" required="true" value="{{isset($vessel) ? $route->city : Input::get('city')}}" @include('form.readonly') >
                      <option value="0">{{trans('route.selectOption')}}</option>
                      @if(isset($view))
                        @foreach($country as $key => $value)
                          <option {{(isset($vessel)) && ($vessel->city == $value->id) ? 'selected' : Input::get('city') == $value->id ? 'selected' : '' }} item="{{$value->toInnerJson()}}" value="{{$value->id}}">{{$value->name}}</option>
