  ...
  <fieldset class="form">
    ...
    <div class="form-group row @include('errors.field-class', ['field' => 'country'])">
      ...
      <div class="col-lg-9">
        ...
        <select style="width:100%;" class="form-control" id="ics_city_country" name="country" required="true" value="{{isset($city) ? $city->country : Input::get('country')}}" @include('form.readonly') >
    ...
    <div class="form-group row @include('errors.field-class', ['field' => 'state'])">
      <label class="col-lg-3 control-label">{{trans('state.state')}}</label>
      <div class="col-lg-9">
        @if(!isset($readonly) || $readonly == false)
        <select style="width:100%;" class="form-control" id="ics_city_state" name="state" required="true" value="{{isset($city) ? $city->state : Input::get('state')}}" @include('form.readonly') >
          <option value="0">{{trans('city.selectOption')}}</option>
          @foreach($state as $key => $value)
            <option {{(isset($city)) && ($city->state == $value->id) ? 'selected' :  Input::get('state') == $value->id ? 'selected' : '' }} item="{{$value}}" value="{{ucwords($value->id)}}">{{ucwords($value->name)}}</option>
          @endforeach
        </select>
        @else
        <input class="form-control" placeholder="" name="" type="text" required="true" value="{{isset($city) ? $city->state : Input::get('state')}}" @include('form.readonly')>
        @endif
