...
<div class="btn-group" role="group" >
  <a href="{{asset('account/prealert/new')}}" class="btn btn-primary" data-toggle="tooltip" title="{{trans('messages.newprealert')}}">
          ...
          @foreach($prealerts as $key => $prealert)
            @if(!isset($filter))
              @if($prealert->complete == HConstants::RESPONSE_NULL)
                <tr>
                  <td style="text-align:center">{{$prealert->code}}</td>
                  <td style="text-align:center"><a class="infoRd" href="javascript:icsViewPrelert({{$prealert->id}})">{{$prealert->order_service}}</a></td>
                  <td style="text-align:center">{{$prealert->provider}}</td>
                  <td style="text-align:center">{{$prealert->getCourier->name}}</td>
                  <td style="text-align:center">{{$prealert->date_arrived}}</td>
